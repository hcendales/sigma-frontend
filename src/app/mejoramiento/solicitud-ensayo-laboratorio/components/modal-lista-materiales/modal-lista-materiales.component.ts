...
@Component({
  selector: 'app-modal-lista-materiales',
  templateUrl: './modal-lista-materiales.component.html',
  styleUrls: ['./modal-lista-materiales.component.scss']
})
export class ModalListaMaterialesComponent implements OnInit {
  
  
  selection: any = new SelectionModel<any>(true, []);
  dataSource: MatTableDataSource<any> = new MatTableDataSource();
  displayedColumns: string[] = ['id_tipo_material_ensayo', 'descripcion_tipo_grupo', 'fuente_material'];
  ...
  filterValues = {
    id_tipo_material_ensayo: '',
    descripcion_tipo_grupo: '',
  };

  constructor(
    public dialogRef: MatDialogRef<ModalListaMaterialesComponent>,
  ...
  ngOnInit() {
    ...
    this.search2.valueChanges
      .subscribe(
        valor2 => {
          this.filterValues.descripcion_tipo_grupo = valor2;
  ...
  async listarData(itm: string): Promise<any> {
    ...
    await this.solicitudEnsayoLaboratorioService.buscarTipoMateriales(itm).then((resp: any) => {
      ...
      this.dataSource.data.map(element => {
         ...
         */
        let texto: string = "";
        if (this.data.invs !== undefined) 
        {
          this.data.invs.find((input: any) => {
            if (input.id_tipo_material_ensayo === element.id_tipo_material_ensayo)
            {
              count ++
              this.arrayChecked.push(element);
              texto = input.fuente_material;
              element.checked = true;
            }
          });
        }
        element['fuente_material'] = texto;
      });
  ...
  creaFiltroXColumna() {
    ...
    this.dataSource.filterPredicate = ((data: any, filter: any) => {
      ...
      return (data.id_tipo_material_ensayo    || '').toString().toLowerCase().indexOf(searchTerms.id_tipo_material_ensayo) !== -1
          && (data.descripcion_tipo_grupo     || '').toString().toLowerCase().indexOf(searchTerms.descripcion_tipo_grupo)  !== -1;
    });
  ...
  onAsociar(): void {
    ...
    const validator = this.arrayChecked.find((element: any) => element.fuente_material === "" );
