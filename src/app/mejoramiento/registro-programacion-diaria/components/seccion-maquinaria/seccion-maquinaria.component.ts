...
export class SeccionMaquinariaComponent implements OnInit {
  ...
  @Input() set programacion_diaria(row: any) {
    if (row !== undefined) {
      let isValidHours = this.valJornada.find((e:any) => e.jornada === row.id_tipo_jornada )

      this.min = isValidHours.ini
      this.max = isValidHours.fin
  ...
  async ngOnInit() {
    ...
    // RN - se elimina maquinaria Menor
    ...
    this.listaEquipo = this.getListas(this.listas[2], 'id_tipo_clase_equipo', 'id_tipo_equipo', 'descripcion_tipo_equipo').filter(e => e.id_tipo !== 14254);
